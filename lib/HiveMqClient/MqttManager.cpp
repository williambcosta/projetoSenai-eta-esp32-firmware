...
void MqttManager::begin(const char* topicoComandos, const char* topicoDados) {
...
void MqttManager::mqttCallback(char* topico, byte* payload, unsigned int tamanho) {
    if (_instance != nullptr) {                          // Verifica se a instancia existe
        _instance->handleMsg(topico, payload, tamanho);  // Redireciona os dados para a função real da classe que lida com a mensagem
    }
}

// Função que processa a mensagem recebida
void MqttManager::handleMsg(char* topico, byte* payload, unsigned int tamanho) {
    String menssagem = "";
...
bool MqttManager::publish(const char* payload) {
    ...
    if (client.publish(mqtt_topico_dados, payload)) {  // Tenta publicar a mensagem
